...
const dateRangeSchema = z.custom(
	(value) => {
		try {
...
export const UserSchema = z.object({
	name: z
		...
		.max(20, validationMessages.name.maxLength),
	...
	favoriteFood: z
		.enum(['apple', 'banana', 'coconut', 'peach', 'mango'])
		.refine((value) => value !== '', validationMessages.favoriteFood.invalid),
	location: z
		.enum(['Neustrelitz', 'Krakow am See', 'Rostock', 'Greifswald'])
		.refine((value) => value !== '', validationMessages.location.invalid),
	towertype: z
		.enum(['slubehome', 'slubetower'])
		.refine((value) => value !== '', 'Towertype is required.'),
	slider: z.number().min(0, validationMessages.slider.min).max(100, validationMessages.slider.max),
	toggle: z
		.boolean()
		.refine((value) => typeof value === 'boolean', validationMessages.toggle.isBoolean),
