<script lang="ts">
  ...
  $: filteredCalls = $timeUsageQuery.data?.calls?.filter(call => {
...
<div class="p-4 space-y-8">
  ...
  {:else if $timeUsageQuery.error}
    <div class="card variant-filled-error p-4" role="alert">
  ...
  {:else}
    ...
    <div class="card variant-filled-surface">
      <header class="card-header p-4 flex justify-between items-center border-b border-surface-600">
        <h2 class="h2">Call History</h2>
        <div class="flex gap-4">
          <select
            bind:value={statusFilter}
            class="select variant-soft"
          >
