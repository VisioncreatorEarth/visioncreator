...
<div class="h-screen w-screen overflow-hidden bg-slate-900 text-white flex flex-col">
  <!-- Header with logo and navigation -->
  <header class="flex justify-between items-center py-3 px-5 bg-gray-900/70 backdrop-blur-sm z-10 fixed top-0 left-0 right-0 border-b border-gray-800/50">
    <div class="flex items-center">
      <!-- Visioncreator Logo -->
      <div class="logo w-10 h-10 mr-3">
        <div class="w-full h-full flex items-center justify-center">
          <div class="w-8 h-8 relative transform rotate-45">
            <div class="absolute inset-0 bg-gradient-to-br from-green-400 to-yellow-400 rounded-sm"></div>
            <div class="absolute top-1 right-1 w-2 h-2 bg-teal-400 rounded-sm transform -rotate-45"></div>
          </div>
        </div>
      </div>
      <div>
    ...
    <div class="flex items-center space-x-4">
      <button 
        class="p-2 rounded-full hover:bg-gray-800/50 {currentSlideIndex === 0 ? 'opacity-30 cursor-not-allowed' : ''}" 
        on:click={prevSlide}
      ...
      <button 
        class="p-2 rounded-full hover:bg-gray-800/50 {currentSlideIndex === slides.length - 1 ? 'opacity-30 cursor-not-allowed' : ''}"
        on:click={nextSlide}
      ...
      <a href="/presentation/preview" class="px-3 py-1 text-sm hover:bg-gray-800/50 rounded">
        Exit to Preview
      </a>
    </div>
  </header>
  ...
  <main class="relative presentation w-full bg-slate-900 overflow-y-auto overflow-x-hidden h-full pt-16">
    <!-- Background gradient -->
    ...
    <div class="fixed right-6 top-1/2 transform -translate-y-1/2 z-50 hidden md:block">
      <div class="flex flex-col space-y-3">
        {#each slides as _, i}
          <button 
            class="w-3 h-3 rounded-full transition-all duration-200 {i === currentSlideIndex ? 'bg-yellow-400' : 'bg-gray-500/50 hover:bg-gray-400/70'}"
    ...
    <div class="min-h-screen">
      {#each slides as slide, i}
        <section 
          ...
          class="min-h-screen w-full py-16 {i !== slides.length - 1 ? 'border-b border-slate-800/50' : ''}"
          id="section-{i}"
        >
    ...
    <button 
      class="fixed bottom-4 right-4 p-3 rounded-full bg-slate-800/50 hover:bg-slate-800/80 text-slate-400 hover:text-slate-100 transition-all duration-300 z-50 backdrop-blur-sm"
    ...
    {#if currentSlideIndex === 0}
      <div class="fixed bottom-8 left-1/2 transform -translate-x-1/2 z-40 animate-bounce hidden md:block">
        <div class="flex flex-col items-center">
          <span class="text-gray-400 text-sm mb-2">Scroll Down</span>
